...
uint8_t spiflash_is_busy(void) {
    ...
    if (res != 0)
#ifdef SPI_DBG
    kprintf("SPI status Register is %d\n", res);
#endif
...
#ifdef SPI_DBG
    kprintf("\tCommand is %x, Adress: %04x\n", cmd, buff);
#endif
